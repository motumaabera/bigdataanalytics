...
The original LAB_1 word frequency table was exported as `word_frequency_lab1.csv`. The most frequent words were `data, education, technology, ai, better`. These terms show that the corpus is mainly about education, technology, data, and machine learning. The word cloud visually confirmed the same pattern. The word length distribution showed that many words were medium-length content words, which is expected after stopword removal. TF-IDF produced a numerical matrix showing the importance of each term in each sentence, while bigram TF-IDF captured meaningful phrases such as machine learning, data science, and decision making.
